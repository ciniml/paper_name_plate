...
- **I2C バスは起動時 100 kHz → bring-up 後に PM1/IOE1 の SPD を立てて 400 kHz 運用**（タグエミュレーションの応答期限のため）。PM1/IOE1 の SPD ビット(400 kHz)は常時給電のため ESP32 リセット後も残り、100 kHz アクセスが部分的に失敗する。`main.rs` の `recover_i2c_speed` が 400 kHz で SPD を戻す。`I2C_CFG` 書き込み直後は 5 ms 待つ（次トランザクションが NACK になる）。
...
- 表示: 名前・肩書・所属・URL（u8g2-fonts）。2026-09-02 表示+読み取り実機確認済み。フラッシュ永続化は 2026-09-03 実機確認済み（nvs 領域 0x9000）。日本語フォントは 2026-09-04 対応済み（u8g2 b16_t_japanese2、名前は 2x スケール）。
...
- **BLE から公開内容を設定**: CTRL `11 len crc32 ack_every` で始める同じウィンドウ転送で UTF-8 の `name\nemail\ngithub\nx\nnote\nurl` を送り、COMMIT で `PlateContent::apply_plain` → NDEF 正規化・保存・再描画。現在値は CONTENT characteristic（`...05`、読み出し、長い読み出し対応）。名前・メール・GitHub・X・メモ合計 240 B、URL 200 B まで（NDEF 短レコードの制限）。Web ページに編集フォーム（読み込み／書き込み）。
...
- **deep sleep（無操作時）**: 無操作（タッチ/ボタン/NFC フィールド/BLE 通信なし）120 秒 かつ 起動 60 秒後 かつ **USB ホスト未接続**（USB-JTAG の SOF フレームカウンタで判定）で `Rtc::sleep_deep`。起床要因は RTC-IO: NFC IRQ(GPIO6 High) / タッチ INT(GPIO4 Low) / ボタン A,B(GPIO2,3 Low) と 6 時間タイマ。起床＝リセットで、`run()` が `reset_reason == CoreDeepSleep` を見て e-paper の初期リフレッシュとフロントライト点滅を省く（パネルは表示を保持）。NFC チップは IOE1 経由で通電したまま target モードで待つのでフィールド検出で起きる（起動 1〜2 s 後に応答）。light sleep は esp-hal 1.1 でこの基板では戻ってこないため不採用（JOURNAL 2026-09-05）。ベンチ: BLE CTRL `05` で 15 秒タイマの deep sleep。
